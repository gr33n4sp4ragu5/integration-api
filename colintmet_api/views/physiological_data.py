...
class PostPhysiologicalData(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            insert_physiological_data(db_connection, physiological_data, request.user.id)
            return Response({"status": "Successfully added physiological data"}, status = status.HTTP_201_CREATED)
        except Exception as error:
            logging.error("Error while adding physiological data. Error is \n %s", error)
